## File name to use in search: read_data_run_statistical_model_full_gender_regions_w_controls_model_object.R ##
...
# without any independent variables, this model allows seeing the distribution of talents over the dependent variable

m0 <- multinom(mobility_status2 ~ 1, data = talents, probit = TRUE, maxit = 300)

m1 <- multinom(mobility_status2 ~ ctrl_obs_2cat + acadmic_age, data = talents, probit = TRUE, maxit = 300)

m2 <- multinom(mobility_status2 ~ ctrl_obs_2cat * gender + acadmic_age, data = talents, probit = TRUE, maxit = 300)

m3 <- multinom(mobility_status2 ~ ctrl_obs_2cat * region2use + acadmic_age, data = talents, probit = TRUE, maxit = 300)

mprobit1 <- multinom(mobility_status2 ~ ctrl_obs_2cat * gender * region2use + acadmic_age, data = talents, probit = TRUE, maxit = 300)

# ============================
#### Export data for figures 2 and 3 ####
# ============================
print('ggpredict started')
# predicted probabilities
# write predicted probabilities to CSV to visualize in Python
m1_res = tibble(ggpredict(mprobit1, terms = c("ctrl_obs_2cat", "gender", "region2use")))


print(head(m1_res))

write_csv(x = m1_res, file = args$output[[1]])
print('ggpredict done, probability table exported in: ')
print(args$output[[1]])

# ============================
#### Export all models as RData to prevent re-running models all the models ####
# ============================

save(list=c("talents", "m0", "m1","m2","m3","mprobit1"),file=args$output[[2]])

# to have results in log
print('Stat model objects saved in: ')
print(args$output[[2]])
