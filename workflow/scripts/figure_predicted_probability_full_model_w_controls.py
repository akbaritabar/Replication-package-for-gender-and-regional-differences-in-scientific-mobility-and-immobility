## File name to use in search: figure_predicted_probability_full_model_w_controls.py ##
...
talents2 = pd.read_csv(args.input[0])

lg(f"Data read from '{args.input[0]}'")
lg(f"A few rows: '{talents2.tail()}'")

# add percent for predicted probability
talents2['pred_percent'] = pd.Series(round(talents2['predicted'] * 100, ndigits=0)).astype(int)

talents2["x"] = pd.Categorical(talents2["x"], categories=talents2["x"].unique())
talents2["response.level"] = pd.Categorical(talents2["response.level"], categories=talents2["response.level"].unique())
talents2["group"] = pd.Categorical(talents2["group"], categories=talents2["group"].unique())
talents2["facet"] = pd.Categorical(talents2["facet"], categories=talents2["facet"].unique())
...
fig2save = (
    gg.ggplot(talents2, gg.aes(x="response.level", y="pred_percent", group='x', fill='x'))
    ...
    # + gg.scale_y_continuous(breaks=[0, 10, 30, 50], labels=[0, 10, 30, 50], limits=[0, 65])
    + gg.facet_wrap('~group+facet') #, ncol=3)
    + gg.coord_flip()    
    + gg.theme_bw(base_family='Verdana')
    + gg.theme(legend_position=('bottom'), legend_direction='horizontal',
            axis_text_x=gg.element_text(size=8, angle=0),
             axis_text_y=gg.element_text(size=8),
             axis_title_x=gg.element_text(size=9),
             axis_title_y=gg.element_text(size=9),
             strip_text_x=gg.element_text(size=5, weight='bold'),
             figure_size=(7, 8)
            )
